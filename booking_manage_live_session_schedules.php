...
$check = $wpdb -> get_results("select count(*) as counting from booking_school_session where date ='".$_POST['date']."' and instructor = '".$_POST['instructor_id']."' and flight = '".$_POST['flight_id']."'");
	if($check[0]->counting < 1){
			$count = count($_POST['time_from']);
			for($i=0;$i < $count;$i++){
				$wpdb->insert('booking_school_session_schedules',array('instructor_id' => $_POST['instructor_id'],'flight_id' => $_POST['flight_id'],'title'=>$_POST['title'],'time_from'=>$_POST['time_from'][$i],'time_to'=>$_POST['time_to'][$i],'date'=>$_POST['date']),array('%d','%d','%s','%s','%s','%s'));
			}
	}
}
if($_POST['updateschedule'] == "Save"){
$wpdb->update( 'booking_school_session_schedules', array('instructor_id' => $_POST['instructor_id'],'flight_id' => $_POST['flight_id'],'title'=>$_POST['title'],'time'=>$_POST['time'],'occurance'=>$_POST['occurance'],'day'=>$_POST['day']), array('id' => $_GET['edit']) ); 
}
if($_GET['delete']){
$wpdb->delete('booking_school_session_schedules',array('id'=> $_GET['delete'])); 
}?>
<div style="text-align:center"><h3 class="booking_h3">Manage Instructor Schedules</h3></div>
<?php
if($_GET['edit']){
$edited = $wpdb -> get_results("select * from booking_school_session_schedules where id =".$_GET['edit']);
?>
...
<table class="gridtable" >
        ...
        <?php  $i=1;
	$rows1 = $wpdb -> get_results("select * from booking_school_session_schedules");
	foreach($rows1 as $key=>$row1){		
	
				$rows_inst = $wpdb -> get_results("select * from booking_school_session_instructors where id =".$row1->instructor_id);
				$rows_flight = $wpdb -> get_results("select * from booking_school_session_flights where id = ".$row1->flight_id);
	    ...
	    <td ><?php echo $rows_flight[0]->flight_name; ?></td>
            ...
            <td ><?php echo $date; ?></td>
            <td><!--<a class="booking_button" href="?page=booking-live-session-schedules&edit=<?php echo $Get_id; ?>">Edit</a>--> &nbsp;&nbsp; <a class="booking_button" href="?page=booking-live-session-schedules&delete=<?php echo $Get_id; ?>" onclick="javascript:return confirm('Are you sure to delete it?')">Delete</a></td>
        </tr>
        <?php $i++; 
			}
			?>
