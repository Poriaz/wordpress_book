...
$wpdb->insert('booking_school_session_flights',array('flight_name' => $_POST['name'],'about'=>$_POST['description']),array('%s','%s'));
}
if($_POST['update_flight'] == "Update Flight"){
$wpdb->update( 'booking_school_session_flights', array('flight_name' => $_POST['name'],'about'=>$_POST['description']), array('id' => $_GET['edit']) ); 
}
if($_GET['delete']){
$wpdb->delete('booking_school_session_flights',array('id'=> $_GET['delete'])); 
}
?>
...
<?php
if($_GET['edit']){
$edited = $wpdb -> get_results("select * from booking_school_session_flights where id =".$_GET['edit']);
?>
<h3 class="booking_h3">Edit Flight</h3>
<a href="?page=booking-manage-live-session-flights">Add new</a>
...
?>
<tr>
	<td><?php echo $row->flight_name;?></td><td><?php echo $row->about;?></td><td><a class="booking_button" href="?page=booking-manage-live-session-flights&edit=<?php echo $row->id;?>">Edit</a>&nbsp;&nbsp;<a class="booking_button" href="?page=booking-manage-live-session-flights&delete=<?php echo $row->id;?>" onclick="javascript:return confirm('Are you sure to delete it?')">Delete</a></td>
</tr>
<?php } ?>
</table>
